...
#+BEGIN_SRC emacs-lisp
    ...
    (setq tmux-color "#005f5f")

    (defun update-tmux (color)
        (if (not (string-equal tmux-color color))

        (setq tmux-color color)

        (save-window-excursion
            (shell-command
                (concat "tmux set-option -t $(tmux display-message -p '#S') status-bg \"" color "\" &"))
     )))
    ...
    (lexical-let ((default-color (cons (face-background 'mode-line)
     ...
     (add-hook 'post-command-hook
       (lambda ()
         (let ((color (cond ((evil-normal-state-p)  '("#005f5f"))
                            ((evil-insert-state-p)  '("#5f0000"))
                            ((evil-visual-state-p)  '("#005f87"))
                            ((evil-replace-state-p) '("#000000"))
                            ((evil-emacs-state-p)   '("#005f5f"))
                            )))
